...
def make_pixart_added_cond_kwargs(batch_size, resolution, device, dtype, sample_size=128):
    # PixArt-1024 (sample_size=128) needs resolution / aspect_ratio micro-conditions.
    if sample_size != 128:
        return {"resolution": None, "aspect_ratio": None}
    height = width = resolution
    resolution_t = torch.tensor([height, width], device=device, dtype=dtype).repeat(batch_size, 1)
    aspect_ratio_t = torch.tensor([float(height / width)], device=device, dtype=dtype).repeat(batch_size, 1)
    return {"resolution": resolution_t, "aspect_ratio": aspect_ratio_t}
...
def generate_new(unet,noise_scheduler,latent, noise,encoder_hidden_states, prompt_attention_mask, 
                ...
                resolution=512, sample_size=128):
    ...
    noisy_imgs_list = []
    bsz = latent.shape[0]
    added_cond_kwargs = make_pixart_added_cond_kwargs(
        bsz, resolution, latent.device, latent.dtype, sample_size=sample_size
    )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Path or hub id for the tiny VAE used during PixArt TDM training.",
    )
    parser.add_argument(
        "--cache_pt_path",
        type=str,
        default="./cache.pt",
        help="Torch file with preprocessed prompt cache used by the demo dataloader.",
    )
    parser.add_argument(
        "--prompt_jsonl_path",
        type=str,
        default=None,
        help="Path to JourneyDB-style jsonl with a 'prompt' field (image-free training).",
    )
    parser.add_argument(
        "--prompt_column",
        type=str,
        default="prompt",
        help="Column name for captions in --prompt_jsonl_path.",
    )
    parser.add_argument(
        "--revision",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=900,
        help="Diffusion schedule length for student trajectory anchors.",
    )
    parser.add_argument(
        "--num_student_steps",
        type=int,
        default=4,
        help="Number of few-step student denoising steps (e.g. 4 -> 899/674/449/224, 2 -> 899/449).",
    )
    parser.add_argument(
        "--cfg",
        type=float,
    ...
    if (
        ...
        and args.prompt_jsonl_path is None
        and not Path(args.cache_pt_path).is_file()
    ):
        raise ValueError("Need --prompt_jsonl_path, --cache_pt_path, dataset_name, or train_data_dir.")
...
def main():
    ...
    logger.info(f"Using latent spatial size: {latent_size}x{latent_size}")
    ...
    def build_jsonl_byte_offsets(jsonl_path: Path) -> np.ndarray:
        offsets = []
    ...
    class JsonlPromptDataset(Dataset):
        # Line-based jsonl reader; avoids HuggingFace datasets file locks on shared FS.

        def __init__(self, jsonl_path, offsets, prompt_column, pretrained_model_name_or_path):
        ...
        def _read_prompt(self, idx: int) -> str:
            if self._fh is None:
                self._fh = self.jsonl_path.open("rb")
            self._fh.seek(int(self.offsets[idx]))
            row = json.loads(self._fh.readline().decode("utf-8"))
            if self.prompt_column in row:
                return row[self.prompt_column]
            return row.get("ori_prompt", row.get("text", ""))

        def __getitem__(self, idx):
            text = self._read_prompt(idx)
            text_input = self.tokenizer(
                [text], max_length=120, padding="max_length", truncation=True, return_tensors="pt"
            )
            return text, text_input.input_ids, text_input.attention_mask

    class CustomImagePromptDataset(Dataset):
        def __init__(self, cache_pt_path, pretrained_model_name_or_path, max_train_samples=None):
            self.tokenizer = T5Tokenizer.from_pretrained(
                pretrained_model_name_or_path, subfolder="tokenizer",
            )
            cache_path = Path(cache_pt_path)
            self.data = torch.load(cache_path, weights_only=False)["data"]
        ...
        def __getitem__(self, idx):
            _latents_pth, text, _index = self.data[idx]
            text_input = self.tokenizer(
                [text], max_length=120, padding="max_length", truncation=True, return_tensors="pt"
            )
            return text, text_input.input_ids, text_input.attention_mask

    if args.prompt_jsonl_path:
        jsonl_path = Path(args.prompt_jsonl_path)
        if not jsonl_path.is_file():
            raise FileNotFoundError(f"Missing prompt jsonl: {jsonl_path}")
        index_path = jsonl_path.with_suffix(jsonl_path.suffix + ".offsets.npy")
        if accelerator.is_main_process:
            if not index_path.is_file():
                logger.info(f"Building byte-offset index for {jsonl_path} (one-time, may take a few minutes)...")
                offsets = build_jsonl_byte_offsets(jsonl_path)
                np.save(index_path, offsets)
                logger.info(f"Saved {len(offsets)} prompt offsets to {index_path}")
        accelerator.wait_for_everyone()
        offsets = np.load(index_path, mmap_mode="r")
        if args.max_train_samples is not None:
            offsets = offsets[: args.max_train_samples]
        logger.info(f"Using {len(offsets)} prompts from {jsonl_path}")
        ...
        dataset = CustomImagePromptDataset(
            args.cache_pt_path, args.pretrained_model_name_or_path, args.max_train_samples
        )
    ...
    class Predictor():
        def __init__(self, noise_scheduler, alpha_schedule, sigma_schedule, resolution=1024, sample_size=128):
        ...
        def _added_cond_kwargs(self, noisy_samples):
            return make_pixart_added_cond_kwargs(
                noisy_samples.shape[0], self.resolution, noisy_samples.device,
                noisy_samples.dtype, sample_size=self.sample_size,
            )
        ...
        def predict(self, score_model, noisy_samples, timesteps, encoder_hidden_states, prompt_attention_mask, cfg=None, steps=1,
            ...
            for _ in range(steps):
                added_cond_kwargs = self._added_cond_kwargs(noisy_samples)
                score_pred = score_model(noisy_samples, timestep = timesteps, added_cond_kwargs = added_cond_kwargs,
                                                    encoder_hidden_states = encoder_hidden_states, encoder_attention_mask=prompt_attention_mask, return_dict=False)[0]
                ...
                if cfg is not None:
                    score_uncon_pred = score_model(noisy_samples, timestep = timesteps, added_cond_kwargs = added_cond_kwargs,
                                    encoder_hidden_states = self.uncond_prompt_embeds[:noisy_samples.shape[0]], encoder_attention_mask=self.uncond_attention_mask[:noisy_samples.shape[0]], return_dict=False)[0]
    ...
    predictor = Predictor(
        noise_scheduler, alpha_schedule, sigma_schedule,
        resolution=args.resolution, sample_size=args.latent_size,
    )

    fixed_noise = None
    fixed_c = None
    total_steps = args.total_steps
    num_student_steps = args.num_student_steps
    step_stride = total_steps // num_student_steps
    student_anchor_ts = [
        total_steps - 1 - i * step_stride for i in range(num_student_steps)
    ]
    logger.info(
        f"Student {num_student_steps}-step anchors (total_steps={total_steps}): {student_anchor_ts}"
    )
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet, unet_fake):
                text_ = list(batch[0])
                ls = args.latent_size
                noise = torch.randn([len(text_), 4, ls, ls]).to(torch.float16).to(accelerator.device)
                latents = noise
                ...
                with torch.no_grad():
                    imgs_list, noisy_imgs_list = generate_new(
                        unet, noise_scheduler, noise, noise, encoder_hidden_states, prompt_attention_mask,
                        steps=num_student_steps, return_mid=True, total_steps=args.total_steps,
                        resolution=args.resolution, sample_size=args.latent_size,
                    )
                ...
                with torch.no_grad():
                    ...
                    timesteps_mid_ori = timesteps_g - step_stride + 1
                ...
                if accelerator.sync_gradients:
                    if global_step % 50 == 0:
                        if fixed_c is None:
                            ...
                            ls = args.latent_size
                            fixed_noise = torch.randn([4, 4, ls, ls]).to(torch.float16).to(accelerator.device)
                            fixed_T = T
                        with torch.no_grad():
                            fixed_latents = generate_new(
                                unet, noise_scheduler, fixed_noise, fixed_noise, fixed_c, fixed_mask,
                                steps=num_student_steps, total_steps=args.total_steps,
                                resolution=args.resolution, sample_size=args.latent_size,
                            )
                            fixed_latents_1step = generate_new(
                                unet, noise_scheduler, fixed_noise, fixed_noise, fixed_c, fixed_mask,
                                steps=1, total_steps=args.total_steps,
                                resolution=args.resolution, sample_size=args.latent_size,
                            )
                            ...
                            latent_nstep = generate_new(
                                unet, noise_scheduler, latents, noise, encoder_hidden_states, prompt_attention_mask,
                                steps=num_student_steps, total_steps=args.total_steps,
                                resolution=args.resolution, sample_size=args.latent_size,
                            )
                            images_nstep = vae.decode(latent_nstep.to(vae.dtype) / vae.config.scaling_factor, return_dict=False)[0].clamp(-1,1)*0.5+0.5
                            images_1step = \
                        ...
                        if accelerator.is_main_process:
                            ...
                            save_image(images_noise, f'./{args.output_dir}/fixed_{num_student_steps}step_{global_step}.jpg', normalize=False, nrow=2)
                            save_image(images_fixed1, f'./{args.output_dir}/fixed_1step_{global_step}.jpg', normalize=False, nrow=2)
                            save_image(images_nstep[:4], f'./{args.output_dir}/{num_student_steps}step.jpg', normalize = False, nrow = 2)
